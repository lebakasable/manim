...
FFMPEG *ffmpeg_start_rendering(size_t width, size_t height, size_t fps)
{
  ...
  if (close(pipefd[READ_END]) < 0) {
    TraceLog(LOG_WARNING, "FFMPEG: Could not close read end of pipe on the parent's end: %s", strerror(errno));
  }
...
bool ffmpeg_end_rendering(FFMPEG *ffmpeg)
{
  ...
  if (close(pipe) < 0) {
    TraceLog(LOG_WARNING, "FFMPEG: Could not close write end of pipe on the parent's end: %s", strerror(errno));
  }
